/*=========================================================
310 CADASTRO MUNICIPAL • LAYOUT EM COLUNAS V2
Uma linha por município • sem coluna linha do tempo
=========================================================*/
(function(){
function qmtListaCelula(movs,tipo,campo,fmt){
let a=movs.filter(x=>x.tipo_evento===tipo)
if(!a.length)return'<span class="qmtSub">—</span>'
return`<div class="qmtListaVertical">${a.map((x,i)=>`<div class="qmtCelItem">${fmt?fmt(x[campo],x,i):qmtEsc(x[campo]||'—')}</div>`).join('')}</div>`
}
...
function qmtCssV2(){
if(document.getElementById('qmtStyleV2'))return
let s=document.createElement('style');s.id='qmtStyleV2';s.textContent=`
.qmtTable{min-width:1880px!important;table-layout:auto}.qmtTable th{font-size:9px!important;text-align:center!important;vertical-align:middle!important}.qmtTable td{vertical-align:middle!important;font-size:10px!important}.qmtTable td.qmtMun{text-align:left!important;white-space:normal!important;line-height:1.25}.qmtListaVertical{display:grid;gap:4px}.qmtCelItem{padding:4px 6px;border-bottom:1px solid #e2e8f0;white-space:nowrap;text-align:center}.qmtCelItem:last-child{border-bottom:0}.qmtObsCol{min-width:300px;max-width:430px;white-space:normal!important;line-height:1.45!important;text-align:left!important}.qmtObsLinha{padding:3px 0;border-bottom:1px dashed #e2e8f0;overflow-wrap:anywhere}.qmtObsLinha:last-child{border-bottom:0}.qmtDilacao{padding:5px 7px;background:#fff7ed;border-left:4px solid #d97706;border-radius:6px;line-height:1.35;white-space:normal}.qmtOficioCircular{font-weight:900;white-space:nowrap;color:#0f172a}.qmtDataCol,.qmtPagCol,.qmtEditCol{text-align:center!important;white-space:nowrap}.qmtRespDoc{font-weight:850;color:#0f172a}
`;
...
return`<div class="qmtTopBar"><input id="qmtBusca" class="qmtBusca" placeholder="Pesquisar município..." value="${qmtEsc(filtro)}" oninput="qmtRenderLocal(this.value)"><div class="qmtLegenda"><b>${lista.length}</b> município(s) • acompanhamento documental contínuo</div></div><div class="qmtWrap"><table class="qmtTable"><thead><tr><th>Município</th><th>Situação</th><th>Ofício Circular TCE</th><th>Data envio</th><th>Pág.</th><th>Reiterações TCE / Of. n.</th><th>Data</th><th>Respostas Jurisdicionado / Data</th><th>Doc. n.</th><th>Págs.</th><th>Dilação</th><th>Observações</th><th>Editar</th></tr></thead><tbody>${lista.map(i=>{
let movs=qmtMovMun(i.municipio),sit=qmtSit(i),ini=movs.find(x=>x.tipo_evento==='OFICIO_TCE')
let respostas=movs.filter(x=>x.tipo_evento==='RESPOSTA')
let reiteracoes=movs.filter(x=>x.tipo_evento==='REITERACAO_TCE')
return`<tr><td class="qmtMun">${qmtEsc(i.municipio)}</td><td><span class="qmtStatus" style="color:${sit[2]};background:${sit[3]}">${sit[0]} ${sit[1]}</span></td><td><div class="qmtOficioCircular">n. 16/2026/GABPRES/TCERO</div></td><td class="qmtDataCol">${qmtData(ini?.data_envio||i.dataenviodoc)}</td><td class="qmtPagCol">${qmtEsc(ini?.pagina||i.paginaenviodoc||'—')}</td><td>${reiteracoes.length?`<div class="qmtListaVertical">${reiteracoes.map(x=>`<div class="qmtCelItem qmtRespDoc">${qmtEsc(x.numero_documento||'—')}</div>`).join('')}</div>`:'—'}</td><td class="qmtDataCol">${reiteracoes.length?`<div class="qmtListaVertical">${reiteracoes.map(x=>`<div class="qmtCelItem">${qmtData(x.data_envio||x.data_documento||x.data_recebimento)}</div>`).join('')}</div>`:'—'}</td><td class="qmtDataCol">${respostas.length?`<div class="qmtListaVertical">${respostas.map(x=>`<div class="qmtCelItem">${qmtData(x.data_recebimento||x.data_documento||x.data_envio)}</div>`).join('')}</div>`:'—'}</td><td>${respostas.length?`<div class="qmtListaVertical">${respostas.map(x=>`<div class="qmtCelItem qmtRespDoc">${qmtEsc(x.numero_documento||'—')}</div>`).join('')}</div>`:'—'}</td><td class="qmtPagCol">${respostas.length?`<div class="qmtListaVertical">${respostas.map(x=>`<div class="qmtCelItem">${qmtEsc(x.pagina||'—')}</div>`).join('')}</div>`:'—'}</td><td>${qmtDilatacao(movs)}</td><td class="qmtObsCol">${qmtObsCompleta(i,movs)}</td><td class="qmtEditCol"><button class="qmtEdit" onclick="qmtAutorizar('${qmtEsc(String(i.municipio).replace(/'/g,"\\'"))}')">✏ EDITAR</button></td></tr>`
